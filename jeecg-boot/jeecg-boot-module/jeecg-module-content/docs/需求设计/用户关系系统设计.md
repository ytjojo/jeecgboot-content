...
) COMMENT='用户关系表（注：拉黑为组合操作，会同时设置不看TA/不让TA看/屏蔽搜索/屏蔽私信等多个字段，数据库不单独存储 is_blacklist 字段）';
```
...
补充说明：
- 拉黑（BLACKLIST）为原子组合操作，等价于：不看TA=1、不让TA看=1、屏蔽搜索=1、屏蔽私信=1。
- 取消拉黑等价于将上述四个标志位恢复为0。
...
CREATE TABLE user_relation_stats (
    ...
    blacklist_count INTEGER DEFAULT 0 COMMENT '拉黑数',
    blacklisted_count INTEGER DEFAULT 0 COMMENT '被拉黑数',
...
#### 5.3.1 拉黑用户（完全屏蔽）
```http
POST /api/user/relation/blacklist
Content-Type: application/json
...
#### 5.3.2 取消拉黑
```http
DELETE /api/user/relation/blacklist/{targetUserId}
```
...
#### 5.4.3 获取拉黑列表
```http
GET /api/user/relation/blacklist/list?pageNo=1&pageSize=20
```
...
#### 5.5.1 批量拉黑
```http
POST /api/user/relation/blacklist/batch
Content-Type: application/json
...
#### 5.5.2 批量取消拉黑
```http
DELETE /api/user/relation/blacklist/batch
Content-Type: application/json
