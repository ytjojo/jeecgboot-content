...
## ⛔ 执行前必读：本提示词是方法论，不是补丁文件

```
┌──────────────────────────────────────────────────────────────────┐
│                                                                  │
│   ❌ 禁止行为:                                                    │
│   - 不读 PRD 原文，直接套用本文档中的「已知错误对照表」修改文档        │
│   - 不读数据库 DDL，直接用本文档的字段名列表去修正 PRD interface      │
│   - 不读 Controller/VO 源码，直接按本文档的 API 路径清单修改文档      │
│   - 把本文档当作「一次性补丁脚本」来执行                             │
│                                                                  │
│   ✅ 正确行为:                                                    │
│   - 本文档中的「已知错误」表格仅作参考和交叉验证用途                  │
│   - 每次执行必须重新读取实际源码（PRD → DDL → Controller → VO）      │
│   - 以读取到的实际内容为准，本文档的表格可能已过时                    │
│   - 如果本文档的对照表与实际读取不一致 → 以实际读取为准，并更新本文档   │
│                                                                  │
│   🔴 硬性要求:                                                    │
│   1. 必须先完整读取 PRD 需求文档                                    │
│   2. 必须先完整读取 Flyway DDL SQL 文件                            │
│   3. 必须先读取每个 Controller 和 VO 的源码                         │
│   4. 基于以上三步的读取结果，自行建立对照表                           │
│   5. 用本文档的表格做交叉验证，不一致时以实际源码为准                  │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```

**本文档中的字段对照表、API 路径清单、缺失分析**均基于 2026-06-25 代码快照。代码随时可能变更，**每次执行修正时必须重新验证**。
...
**字段名三层对照表**（⚠️ 2026-06-25 快照，执行时必须重新验证）:
...
**PRD TypeScript interface 字段修正对照表**（⚠️ 2026-06-25 快照，执行时必须重新读取 VO 源码验证）:
...
- `GET /api/v1/content/circle/member_growth/participation?circleId=&userId=` — 连续参与天数（MemberGrowthController）
...
## 四、数据库与 API 缺失分析（PRD 需求对照）

以下逐项检查 PRD 需求是否有对应的数据库表和 API 支撑。**缺失项需在修正文档时明确标注，并反馈给后端补充**。

### 4.1 数据库表缺失

#### 缺失 1：徽章初始化数据不完整（严重 🔴）

**PRD 需求**: 6 种徽章（US-13.3.1 §2 表格）

**数据库实际**: `circle_achievement` 表只 INSERT 了 4 条数据

| PRD 徽章 | DDL 是否初始化 | 备注 |
|---------|-------------|------|
| 持续创作者 (CONTINUOUS_CREATOR) | ✅ ach_001 | 累计发布 10 篇 |
| 优质贡献者 (QUALITY_CONTRIBUTOR) | ✅ ach_002 | 累计 5 篇精华 |
| 活跃参与者 (ACTIVE_PARTICIPANT) | ✅ ach_003 | 近 7 天至少 3 天参与 |
| 圈内新星 (RISING_STAR) | ✅ ach_004 | 近 7 天经验值排名前 10 |
| **内容里程碑** (CONTENT_MILESTONE) | ❌ 缺失 | 累计发布 100 篇可见内容 |
| **社交达人** (SOCIAL_BUTTERFLY) | ❌ 缺失 | 成功邀请 5 人加入圈子 |

**影响**: 前端最多只能展示 4 种徽章，且代码中 `AchievementTypeEnum` 可能只定义了 4 种枚举值。

**修正**: 需补充 Flyway 迁移脚本，INSERT `ach_005` 和 `ach_006`。同时更新 `AchievementTypeEnum` 枚举类。文档中标注「6 种徽章，数据库当前仅初始化 4 种，2 种待补充」。

#### 缺失 2：「社交达人」徽章缺少邀请追踪数据（严重 🔴）

**PRD 需求**: "成功邀请 5 人加入圈子"

**数据库实际**: `content_invite_record`（V3.9.1_59）表结构：

```
content_invite_record:
  inviter_user_id  — 邀请人
...
**问题**: 现有邀请记录是**全局内容社区级别**的（无 `circle_id`），无法统计"邀请用户加入**特定圈子**"的次数。

**修正选项**:
- 选项 A: `content_invite_record` 表增加 `circle_id` 字段（影响已有全局邀请逻辑）
- 选项 B: 新建 `circle_invite_record` 表，圈子专用

**文档标注**: PRD 定义"社交达人"徽章依赖圈子邀请追踪能力，当前数据库不支持。若短期内无法补充，该徽章应标记为「后续迭代」。

#### 缺失 3：无独立参与日历表（中等 🟡）

**PRD 需求**: `streakDetail: boolean[]`（近 7 天每日参与状态实心圆/空心圆），用于「连续参与进度」组件展示 7 天时间轴。

**数据库实际**: 无独立参与日历表。`circle_growth_log` 可按 `biz_date` 推算，但：
- logs 记录的是"行为"（发帖/评论/点赞），需要额外判断是否为"有效参与行为"
- 无当日参与状态的直接记录

**当前 API**: `/participation` 只返回 `Result<Integer>`（连续参与天数），不返回每日明细。

**修正选项**:
- 选项 A: 新建 `circle_participation_calendar(circle_id, user_id, date, has_activity)` 表，每次有效参与行为时 UPSERT
- 选项 B: `/participation` API 改为返回 `List<LocalDate>`（参与日期列表），前端自己算 7 天状态
- 选项 C: `/participation` API 改为返回 `ParticipationVO { days: int, streakDetail: boolean[7] }`

**文档标注**: PRD 描述的 7 天时间轴（每日状态）当前 API 不支持，需后端扩展 `/participation` 接口。

### 4.2 VO 字段缺失（数据库有字段但 VO 未暴露）

| 数据库字段 | 所属表 | VO 是否暴露 | PRD 是否需要 | 严重程度 |
|-----------|--------|-----------|------------|---------|
| `comment_count` | circle_member_growth | ❌ MemberGrowthVO 无 | PRD 未明确要求，但"个人成长页"可能有价值 | 🟢 低 |
| `featured_count` | circle_member_growth | ❌ MemberGrowthVO 无 | PRD 提到"加精"行为 | 🟡 中 |
| `circle_id` | circle_level | ❌ CircleLevelVO 无 | 前端可能需要用于缓存 key | 🟢 低 |
| `user_id` | circle_member_growth | ❌ MemberGrowthVO 无 | 调用方已知 userId | 🟢 低 |

**MemberGrowthVO 额外缺失**（PRD 要求但 VO 无）:

| PRD 字段 | 说明 | 严重程度 |
|---------|------|---------|
| `totalBadges` | 已获得徽章总数 | 🟡 中 — PRD 个人成长页需展示 |
| `totalBadgeCount` | 徽章总数（含未获得） | 🟡 中 — 同上 |

### 4.3 API 返回结构缺失

#### 缺失 4：`/participation` 返回类型不足以支撑 7 天时间轴（中等 🟡）

**当前**: `Result<Integer>` — 仅返回连续参与天数

**PRD 需要**: 7 天每日参与状态（`boolean[7]`），区分「已完成」「未完成」「非统计日」

**建议**: 新增 `ParticipationVO { participationDays: Integer, dailyStatus: List<DayStatus> }`，或让 `/participation` 返回参与日期列表。

#### 缺失 5：Leaderboard 返回扁平数组，缺少包装信息（低 🟢）

**当前**: `Result<List<LeaderboardEntryVO>>` — 扁平数组

**PRD 期望**: `{ entries, currentUser, currentUserGapToPrev, totalCount }`

**实际可行**: 前端从数组中 filter `highlighted === true` 获取当前用户，从该 entry 的 `gap` 获取差距。但 `totalCount` 字段确实缺失。

**建议**: 前端在 API 封装层做结构适配，或后端增加 `totalCount` 到响应中。

### 4.4 CircleLevelController 的 `/benefit` 和 `/config` 接口归属问题（注意 🟡）

`CircleLevelController` 路径为 `/api/v1/content/circle/growth/level/`，但其 `/benefit` 和 `/config` 接口实际返回：

- `/benefit?userId=` → `ContentUserLevelBenefitSummaryVO`（来自**全局用户成长**体系）
- `/config` → `List<ContentUserLevelConfigVO>`（来自**全局用户成长**体系）

**问题**: 这两个接口按 `userId` 查询、返回全局用户等级类型，但挂在圈子等级 Controller 下。可能是以下情况之一：
1. 路径规划：圈子成长体系的等级权益复用全局等级配置
2. 路由混淆：这两个接口实际应放在全局用户成长的 Controller 下

**文档标注**: 需与后端确认这两个接口的归属。若确属圈子成长体系，PRD 需补充说明；若属全局体系，路径应迁移。

---

## 五、修正工作流

> 修正前必须先阅读第四章「数据库与 API 缺失分析」，了解已知缺失项，避免将缺失当成「已实现」来修正文档。
