...
| PRD 概念 | 标准名（数据库字段） | VO/前端应使用的字段名 | 所属表 |
|----------|---------------------|---------------------|--------|
| 经验值 | `exp_points` | `expPoints` | circle_member_growth |
| 贡献值 | `contribution_points` | `contributionPoints` | circle_member_growth |
| 发帖数 | `post_count` | `postCount` | circle_member_growth |
| 评论数 | `comment_count` | `commentCount` | circle_member_growth |
| 精华数 | `featured_count` | `featuredCount` | circle_member_growth |
| 成长分 | `growth_score` | `growthScore` | circle_level |
| 成员得分 | `member_score` | `memberScore` | circle_level |
| 内容得分 | `content_score` | `contentScore` | circle_level |
| 活跃得分 | `activity_score` | `activityScore` | circle_level |
| 行为类型 | `action_type` | `actionType` | circle_growth_log |
| 徽章类型 | `achievement_type` | `achievementType` | circle_achievement / circle_member_achievement |
| 徽章名称 | `name` | `name` | circle_achievement |
| 徽章图标 | `icon_url` | `iconUrl` | circle_achievement |
| 条件描述 | `condition_desc` | `conditionDesc` | circle_achievement |
| 是否撤销 | `revoked` | `revoked` | circle_growth_log / circle_member_achievement |
| 排行榜维度 | `dimension` | `dimension` | circle_leaderboard_snapshot |
| 排行榜周期 | `period` | `period` | circle_leaderboard_snapshot |
| 得分 | `score` | `score` | circle_leaderboard_snapshot |
| 排名 | `rank_num` | `rankNum` | circle_leaderboard_snapshot |
| 快照时间 | `snapshot_time` | `snapshotTime` | circle_leaderboard_snapshot |
| 业务日期 | `biz_date` | `bizDate` | circle_growth_log |
| 业务 ID | `biz_id` | `bizId` | circle_growth_log |
| 圈子 ID | `circle_id` | `circleId` | 所有表 |
| 用户 ID | `user_id` | `userId` | circle_member_growth / circle_growth_log / circle_member_achievement / circle_leaderboard_snapshot |

**PRD 中 TypeScript interface 常见错误对照**:

| PRD interface 中的错误字段名 | 应根据数据库改成的字段名 | 原因 |
|--------------------------|----------------------|------|
| `experience` | `expPoints` | 数据库字段 `exp_points` |
| `contribution` | `contributionPoints` | 数据库字段 `contribution_points` |
| `currentLevel` | `level` | 数据库字段 `level` |
| `nextLevelExp` | `nextLevelThreshold` | PRD 概念是「下一等级门槛」非「下一等级经验」 |
| `streakDays` | `participationDays` | 数据库字段 `participation_days`（若表有此字段） |
| `dailyExpCap` | `dailyExpLimit` | 概念应为「每日上限」 |
| `badgeId` | `achievementType` | 数据库字段 `achievement_type` |
| `badgeName` | `name` | 数据库字段 `name` |
| `badgeIcon` | `iconUrl` | 数据库字段 `icon_url` |
| `value`（排行榜得分） | `score` | 数据库字段 `score` |
| `rank`（排行榜排名） | `rankNum` | 数据库字段 `rank_num` |
| `isCurrentUser` | `highlighted` | VO 计算字段，非数据库直出 |
| `nearComplete` | `status`（值为 CLOSE） | VO 计算字段，非数据库直出 |

**注意**: 上表「PRD 错误字段名」是指 PRD 中 TypeScript interface 定义的字段名应与数据库对齐。PRD 的**文字描述**（如「展示经验值」「展示贡献值」）不变，只修正 interface 代码块中的字段名。
