...
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 获取勋章分类列表 | GET | `/api/v1/content/user/growth/badge/catalog` | 返回按分类分组的勋章定义和用户授予状态 |
| 获取勋章详情 | GET | `/api/v1/content/user/growth/badge/detail` | query: `badgeCode`，返回勋章条件、进度、授予信息 |
| 获取佩戴勋章 | GET | `/api/v1/content/user/growth/badge/worn` | 返回用户当前佩戴的勋章列表 |
| 保存佩戴设置 | POST | `/api/v1/content/user/growth/badge/wear` | body: `{ badgeIds: string[] }`，最多 5 个 |
| 获取用户佩戴展示 | GET | `/api/v1/content/user/growth/badge/worn` | query: `userId`，返回指定用户佩戴的勋章（他人查看用） |
| 回收勋章（管理员） | POST | `/api/v1/content/user/growth/badge/recycle` | body: `{ grantId, reason }` |

### 5.2 积分相关 API

| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 获取积分余额 | GET | `/api/v1/content/user/growth/summary` | 返回当前余额、今日获取、今日消耗及等级信息（合并接口） |
| 查询积分明细 | GET | `/api/v1/content/user/growth/point/ledger` | params: `type, startTime, endTime, page, pageSize` |
| 获取兑换商品列表 | GET | `/api/v1/content/user/growth/point/exchange/goods` | 返回可兑换商品列表（含库存） |
| 兑换商品 | POST | `/api/v1/content/user/growth/point/exchange` | body: `{ goodsId, quantity, requestId }`，requestId 为前端生成的 UUID，用于幂等校验 |
| 解锁功能 | POST | `/api/v1/content/user/growth/point/feature/unlock` | body: `{ featureCode }` |
| 赠送礼物 | POST | `/api/v1/content/user/growth/point/gift/send` | body: `{ giftId, receiverId, quantity, message }` |

### 5.3 等级相关 API

| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 获取等级信息 | GET | `/api/v1/content/user/growth/summary` | 返回等级、经验值、进度、权益、衰减状态（合并接口，含积分余额） |
| 获取等级配置 | GET | `/api/v1/content/user/growth/level/config` | 返回等级阈值表、权益配置 |
| 获取衰减规则 | GET | `/api/v1/content/user/growth/decay/rule` | 返回衰减规则说明 |
...
export const getBadgeCatalog = () => defHttp.get({ url: '/api/v1/content/user/growth/badge/catalog' });
export const getBadgeDetail = (badgeCode: string) => defHttp.get({ url: '/api/v1/content/user/growth/badge/detail', params: { badgeCode } });
export const getWornBadges = (userId?: string) => defHttp.get({ url: '/api/v1/content/user/growth/badge/worn', params: { userId } });
export const saveWearConfig = (badgeIds: string[]) => defHttp.post({ url: '/api/v1/content/user/growth/badge/wear', data: { badgeIds } });

// src/api/content/point.ts
export const getGrowthSummary = () => defHttp.get({ url: '/api/v1/content/user/growth/summary' });
export const getPointLedger = (params) => defHttp.get({ url: '/api/v1/content/user/growth/point/ledger', params });
export const getExchangeGoods = () => defHttp.get({ url: '/api/v1/content/user/growth/point/exchange/goods' });
export const createExchange = (data: { goodsId: string; quantity: number; requestId: string }) => defHttp.post({ url: '/api/v1/content/user/growth/point/exchange', data });

// src/api/content/growth.ts
export const getLevelConfig = () => defHttp.get({ url: '/api/v1/content/user/growth/level/config' });
export const getDecayRule = () => defHttp.get({ url: '/api/v1/content/user/growth/decay/rule' });
```
