...
4. 执行 `superpowers:finishing-a-development-branch` 时，**严禁**选择 Option 1（Merge locally）合并到 `master`
...
### 秒懂：3 条硬规则
1. **必须在 worktree 中开发**（含 `.claude/skills/` 下的技能文件修改），禁止直接在主分支改代码（启动前 `git worktree list` 确认）
...
**subagent 仅允许执行主 agent 明确指派的任务**，禁止以下自主行为：

| 禁止操作 | 说明 |
|---------|------|
| `openspec archive` / `/opsx:archive` | 归档 change 目录 |
| 删除 `openspec/changes/` 下任何文件或目录 | 包括已归档 change 的清理 |
| `git push` / `git push --force` | 推送到远程 |
| 修改 `AGENTS.md` / `CLAUDE.md` | 项目规则文件 |
| 合并到 `master` 分支 | 硬禁止 |
| 删除 worktree（`git worktree remove`） | 仅主 agent 可清理 |

**read-only subagent 输出边界**（主 agent 派发的 Explore、验证、事实调查类只读 subagent）：
- ✅ 允许：描述问题（文件路径+行号）、指出差异、标注严重级别、提出方向性建议
- ❌ 禁止：输出可落地的代码实现（diff、完整组件代码、可直接替换的代码块）
- 底线：**可以建议，不能实施**。越界代码块浪费 token、干扰主 agent 决策
- 不适用：审核类 agent（`ce-*-reviewer` 等）— 其职责本身包含评估和建议

### subagent 作用域约束（硬约束）

**subagent 的文件操作和搜索范围必须以主 agent 指定的路径为边界**：

| 规则 | 说明 |
|------|------|
| **路径白名单** | 主 agent 派发时必须给出允许操作的文件/目录清单，subagent 不得超出 |
| **禁止跨目录扫描** | 禁止使用 `find /`、`find ~/`、`find ../` 等跳出项目根目录的搜索 |
| **禁止探索无关目录** | 禁止"顺便看看"其他模块、扫描兄弟目录、遍历上级目录 |
| **glob 必须有界** | `**/*` 必须限定在指定子目录内（如 `src/views/circle/**/*`），禁止从项目根 `/` 或 `~` 起手 |
| **新增文件仅限指定目录** | 新建文件必须在主 agent 明确指定的目录内 |

**主 agent 派发 subagent 时**，prompt 中路径必须使用**绝对路径**，并提供明确的文件清单或目录边界。错误示例 vs 正确示例：

```
❌ "翻译 docs/ 下的文件"           → subagent 可能 find 整个项目找 .md
✅ "翻译 docs/guide/ 下这 3 个文件: /abs/path/docs/guide/a.md, b.md, c.md"
```

**subagent 收到任务后**，若发现主 agent 未提供明确路径边界、或边界模糊（如"相关的文件"），**必须先提问澄清**，不得自行猜测范围。

**主 agent 派发 subagent 时必须**：
1. 在 prompt 末尾显式声明本次任务**允许和禁止**的操作范围
2. 至少包含：`禁止执行 openspec archive/delete、git push、修改 AGENTS.md 等未指定的操作`
3. 对可能产生副作用的操作（删除文件、移动目录、git 操作）明确是否需要执行
