...
- 不一次性迁移所有 `/content/user/*` 接口到 `/api/v1/*`，仅在设计中保持后续兼容方向。（已完成迁移，当前路径为 `/api/v1/content/user/*`）
...
**`POST /api/v1/content/user/profile/update` 是统一更新端点**，入参为 `ContentUserProfileUpdateReq`（详见 [Delta 端点设计](#delta-端点设计)）。controller 内部委托 `IContentUserProfileService.updateProfile(...)`，service 内部按字段差异化处理：
...
- 上传完成后调用 `/api/v1/content/user/profile/update` 时把 CDN URL 作为 `avatar` / `homepageBackground` 字段提交；
...
`POST /api/v1/content/user/profile/history/restore?userId=X&historyId=Y` 把历史值作为一次新的 `ContentUserProfileUpdateReq` 提交，重新执行必填校验、`@Pattern` 校验、敏感词审核、唯一性校验和审核规则。恢复成功后同样记录当前值进入历史。
...
| HTTP | 路径 | 入参 | 出参 | 说明 |
|------|------|------|------|------|
| GET | `/api/v1/content/user/profile/detail` | `ownerUserId`, `viewerUserId` | `ContentUserProfileVO` | 资料详情（已裁剪） |
| POST | `/api/v1/content/user/profile/update` | `userId` (query) + `ContentUserProfileUpdateReq` (body) | `ContentUserProfileVO` | 统一资料更新 |
| POST | `/api/v1/content/user/profile/review/handle` | `ContentUserReviewHandleReq` (body) | `Result<?>` | 审核处理（后台） |
| POST | `/api/v1/content/user/profile/privacy/update` | `userId` (query) + `ContentUserPrivacyUpdateReq` (body) | `ContentUserPrivacySettingVO` | 隐私配置更新 |
| POST | `/api/v1/content/user/profile/homepage/update` | `userId` (query) + `ContentUserHomepageUpdateReq` (body) | `Result<ContentUserProfileVO>` | 主页配置更新 |
| POST | `/api/v1/content/user/profile/homepage/defaults/restore` | `userId` (query) | `Result<ContentUserProfileVO>` | 恢复主页默认 |
| GET | `/api/v1/content/user/profile/homepage/modules` | `userId` (query) | `List<ContentUserHomepageModuleVO>` | 主页模块列表 |
| GET | `/api/v1/content/user/profile/badge/list` | `userId` (query) | `List<ContentUserVerificationBadgeVO>` | 认证标识列表 |
| GET | `/api/v1/content/user/profile/badge/detail` | `badgeId` (query) | `ContentUserVerificationBadgeVO` | 认证标识详情 |
| GET | `/api/v1/content/user/profile/history/list` | `userId`, `historyType` (query) | `List<ContentUserProfileHistoryVO>` | 历史记录列表 |
| POST | `/api/v1/content/user/profile/history/restore` | `userId`, `historyId` (query) | `Result<ContentUserProfileVO>` | 恢复历史 |
...
- [本期缺失独立上传端点] → 头像/背景图约束（5MB、JPG/PNG/WebP）由前端 + OSS 联合保证；如果未来需要服务端二次校验，可通过新增 `POST /api/v1/content/user/profile/asset/validate` 端点补齐。
