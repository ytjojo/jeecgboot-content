...
| 服务层已有但未暴露 | 5 | `IContentUserSupportService` 已实现，仅需添加控制器端点 |
| 完全缺失 | 12 | 服务层和控制器均未实现，需全新开发 |
...
以下方法在 `IContentUserSupportService` 接口已定义且 `ContentUserSupportServiceImpl` 已实现，但控制器中缺少对应的 HTTP 端点。**仅需在控制器中添加 HTTP 映射即可**。
...
按优先级顺序实现：
1. P0: 用户端举报列表、举报详情、申诉详情
2. P1: 撤回功能、帮助中心接口、更新日志端点
3. 服务层已有方法暴露为 HTTP 端点（5个，工作量最小）
