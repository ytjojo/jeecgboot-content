...
- [x] 1.1 在内容社区模块路由下新增 `/content/profile/edit`、`/content/profile/homepage-settings`、`/content/profile/privacy`、`/content/profile/history` 四个路由配置
- [x] 1.2 创建编辑资料页骨架组件 `views/profile/edit/index.vue`，包含顶部导航栏、头像区域、表单区域、底部操作栏
- [x] 1.3 创建主页设置页骨架组件 `views/profile/homepage-settings/index.vue`，包含背景图设置区、主题色设置区、预览区
- [x] 1.4 创建隐私设置页骨架组件 `views/profile/privacy/index.vue`，按 7 个分组折叠展示 15 个 `*Visibility` 字段 + 2 个 Boolean
- [x] 1.5 创建历史记录页骨架组件 `views/profile/history/index.vue`，包含 Tabs 切换（昵称/头像）、当前值展示区、历史列表
...
- [x] 5.1 创建 `AvatarCropper` 组件，集成 `cropperjs`，头像 1:1 比例、背景图 16:9 比例
- [x] 5.2 实现文件校验：格式（JPG/PNG/WebP）、大小（≤5MB）、非空
- [x] 5.3 实现裁剪弹窗 UI：图片预览区、缩放控制条、底部操作按钮
- [x] 5.4 实现 OSS 上传流程：使用 STS 临时凭证，禁止硬编码 AccessKey
- [x] 5.5 实现移动端适配：全屏展示裁剪区域

## 6. 主页设置页实现

- [x] 6.1 实现背景图上传，复用 AvatarCropper 组件（16:9 比例），支持更换背景和恢复默认
...
- [x] 8.1 创建 `VerificationBadge` 组件，Props 接收 `badges: ContentUserVerificationBadgeVO[]`
- [x] 8.2 维护 `visualStyleKey` 字典：`INDIVIDUAL`（蓝对勾）、`ENTERPRISE`（金徽章）、`CREATOR`（紫星）、`OFFICIAL`（红盾）、`REAL_NAME`（灰盾）、`MOBILE`（绿手机）、`EMAIL`（绿邮箱）
...
- [x] 9.1 覆盖 15 个 `*Visibility` 字段：按 D7 分组（基础资料/扩展/主页/认证/活动/在线状态/布尔开关）
- [x] 9.2 基础资料组 (5)：`bioVisibility` / `genderVisibility` / `birthdayVisibility` / `regionVisibility` / `professionVisibility`
- [x] 9.3 扩展资料组 (1)：`personalLinkVisibility`
- [x] 9.4 主页组 (3)：`homepageBackgroundVisibility` / `themeColorVisibility` / `homepageModuleVisibility`
- [x] 9.5 认证组 (2)：`certificationVisibility` / `verificationBadgesVisibility`
...
- [x] 11.1 实现所有页面的响应式断点适配（xs/sm/md/lg/xl），PC 端居中 640px，移动端全宽
- [x] 11.2 实现通用交互：表单实时校验（防抖 300ms）、防重复提交、危险操作确认弹窗、成功消息提示
- [x] 11.3 实现 iPhone Safe Area 适配：底部操作栏 padding-bottom: env(safe-area-inset-bottom)
- [x] 11.4 实现认证标识弹窗的响应式切换：PC 端 Modal，移动端 Drawer

## 12. 测试与验证

- [x] 12.1 编写编辑资料页单元测试：表单校验、OSS 上传、审核状态展示
- [x] 12.2 编写头像裁剪组件单元测试：文件校验、裁剪参数、上传流程
- [x] 12.3 编写认证标识组件单元测试：排序逻辑、折叠策略、详情弹窗、visualStyleKey 字典
- [x] 12.4 编写隐私设置页单元测试：15 个 visibility 字段、onlineStatusVisibility 特殊枚举
- [x] 12.5 编写历史记录页单元测试：historyType 切换、恢复操作
- [x] 12.6 全量测试运行，确保 100% 通过
