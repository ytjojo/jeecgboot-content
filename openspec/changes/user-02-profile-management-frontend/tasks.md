...
- [ ] 4.4 实现 OSS 客户端直传：选文件 → 客户端校验（格式 + 大小） → cropperjs 裁剪 → 上传 → 回填 CDN URL → 调用 `POST /profile/update`
...
- [ ] 6.5 实现保存（`POST /content/user/profile/homepage/update`）和恢复默认（`POST /content/user/profile/homepage/defaults/restore`）功能
...
- [ ] 10.5 恢复操作：确认弹窗 → `POST /content/user/profile/history/restore?userId=X&historyId=Y` → 成功提示
