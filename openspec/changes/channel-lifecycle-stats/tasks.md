...
- [ ] 6.1 实现不活跃频道识别逻辑（连续 6 个月无活动）
- [ ] 6.2 实现自动归档流程（通知后 1 个月无改善）
- [ ] 6.3 实现组织频道归档通知逻辑
- [ ] 6.4 实现手动归档和申请归档流程
- [ ] 6.5 实现归档频道从发现入口排除
...
- [ ] 7.1 创建 ChannelMergeBiz：合并流程编排（跨聚合）
- [ ] 7.2 实现合并申请逻辑：目标频道校验、影响范围展示
- [ ] 7.3 实现组织频道合并审批流程
- [ ] 7.4 实现合并执行逻辑：内容和订阅关系迁移
- [ ] 7.5 实现合并后源频道状态变更和目标频道入口展示
- [ ] 7.6 实现合并审计日志记录

## 8. 违规处理能力（channel-violation-handling）

- [ ] 8.1 实现限制推荐逻辑：从公共推荐流排除
- [ ] 8.2 实现强制隐藏逻辑：对外不可见
- [ ] 8.3 实现永久关闭逻辑：二次确认、不可恢复
- [ ] 8.4 实现违规处理通知和申诉入口
- [ ] 8.5 实现违规处理审计日志记录

## 9. 不活跃治理能力（channel-inactivity-governance）

- [ ] 9.1 创建定时任务：每日扫描不活跃频道
- [ ] 9.2 实现不活跃提醒通知逻辑
- [ ] 9.3 实现个人频道自动归档逻辑
- [ ] 9.4 实现组织频道通知管理员逻辑
- [ ] 9.5 实现恢复活跃解除风险状态逻辑
