...
- `src/api/content/filterRule.ts`：屏蔽规则相关
  - `POST /content/user/filter-rule` — 添加规则
  - `POST /content/user/filter-rule/delete` — 删除规则
  - `POST /content/user/filter-rule/batch-delete` — 批量删除
  - `GET /content/user/filter-rule/list` — 规则列表
