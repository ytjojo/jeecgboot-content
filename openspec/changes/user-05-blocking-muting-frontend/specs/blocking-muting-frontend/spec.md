...
#### Scenario: 重复拉黑
- **WHEN** 用户对已拉黑的用户再次执行拉黑操作
- **THEN** 后端幂等处理，前端不报错，显示"已拉黑"（与首次操作一致）
...
#### Scenario: 重复屏蔽
- **WHEN** 用户对已屏蔽的用户再次执行屏蔽操作
- **THEN** 后端幂等处理，前端不报错，显示"已屏蔽"（与首次操作一致）
...
#### Scenario: 乐观更新视觉表现
- **WHEN** 用户点击「不感兴趣」
- **THEN** 内容卡片立即从信息流移除（视觉上无延迟），原卡片位置显示气泡选项
- **WHEN** 用户未选择任何气泡选项直接关闭气泡
- **THEN** 气泡消失，卡片位置保持空白（不恢复）

#### Scenario: 内容无 category 和 topics 时的降级展示
- **WHEN** 内容数据中 `category` 为空且 `topics` 数组为空
...
### Requirement: 边界场景处理
系统 SHALL 正确处理网络异常和操作组合等边界场景。

#### Scenario: 网络异常时拉黑/屏蔽操作
- **WHEN** 用户确认拉黑/屏蔽但网络请求失败
- **THEN** 显示全局错误消息"操作失败，请重试"，弹窗关闭，用户上下文保留（不跳转、不刷新）

#### Scenario: 同时拉黑和屏蔽同一用户
- **WHEN** 用户对同一目标用户先执行屏蔽再执行拉黑（或反之）
- **THEN** 两种操作独立生效，拉黑优先级高于屏蔽（被拉黑后屏蔽状态仍保留但不生效，解除拉黑后屏蔽自动恢复生效）
