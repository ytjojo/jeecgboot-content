...
- **API 依赖**: 依赖后端 10 个接口（通知设置 3 个、隐私设置 2 个、安全设置 2 个、第三方授权 3 个），接口路径前缀 `/content/user/`。其中隐私设置 GET 端点尚未实现，需后端补充（详见 `backend-issues.md`）。安全设置更新端点 `POST /api/v1/content/user/settings/security/update` 后端已实现
