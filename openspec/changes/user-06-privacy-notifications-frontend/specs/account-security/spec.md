...
#### Scenario: 正常加载安全设置
- **WHEN** 用户进入账户安全页
- **THEN** 调用 `GET /api/v1/content/user/settings/security`，展示四个安全功能卡片，状态从接口返回的 Boolean 字段获取
...
> **后端状态**: `GET /api/v1/content/user/settings/security` 已实现，可读取 `loginAlertEnabled` 状态。但 `POST /api/v1/content/user/settings/security/update` 尚未实现，需后端补充更新端点。详见 `backend-issues.md`。

#### Scenario: 切换登录提醒开关
- **WHEN** 用户点击登录提醒的 Switch 开关
- **THEN** 调用 `POST /api/v1/content/user/settings/security/update` 接口更新 `loginAlertEnabled` 状态，成功后卡片状态描述更新
