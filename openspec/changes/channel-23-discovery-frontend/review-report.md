...
### BLOCK-2: 4 个后端 API 缺失阻塞前端功能

**严重程度**: BLOCK

根据 backend-issues.md 和 verification-review.md，以下后端 API 尚未实现：

| 编号 | 缺失 API | 阻塞的前端功能 | 优先级 |
|------|---------|--------------|--------|
| BI-1 | `GET /content/channel/discovery/home`（聚合接口 Controller） | 发现页核心功能 | P0 |
| BI-2 | `POST /content/channel/category/enable` | 分类管理 - 启用分类 | P0 |
| BI-3 | `POST /content/channel/tag/update` | 标签管理 - 编辑标签 | P0 |
| BI-4 | `POST /content/channel/search/feedback` | 搜索结果反馈 | P1 |
| BI-5 | `GET /content/channel/editorial-pick/page` | 精选管理 admin 分页列表 | P1 |
| BI-6 | `ContentChannelBrowseController.browseByCategory()` 方法体 | 分类浏览页 | P2 |

**建议**: 
- P0 缺失项（BI-1/2/3）必须在前端实现前完成后端补充
- P1 缺失项（BI-4/5）可先实现前端 UI，使用 mock 数据或跳过对应功能
- P2 缺失项（BI-6）可先用模拟数据开发分类浏览页
...
channel-23-discovery-frontend 的规范文档结构完整、PRD 覆盖率高，但存在 4 个 BLOCK 级问题需要在 apply 前解决：

1. **BLOCK-1/BLOCK-3**: 测试任务和测试策略完全缺失，违反项目 TDD 硬规则
2. **BLOCK-2**: 3 个 P0 后端 API 缺失（聚合接口、分类启用、标签编辑）阻塞核心功能
...
3. 更新前端 PRD 中的 API 路径（搜索、排行榜、分类管理）
4. 在前端 PRD API 列表中补充 userId 参数说明
5. 确认 BI-1/BI-2/BI-3 的后端完成状态，若未完成则在 tasks.md 中标注为阻塞项

**Apply 后建议完成**:
1. 在各受后端缺失影响的 spec 中添加 NOTE 标注
