...
channel-discovery spec 定义了"未登录用户查看发现页"场景，但未明确说明未登录状态下推荐接口如何处理 userId（传空？不传？使用冷启动接口？）。design.md 提到冷启动使用 `GET /api/v1/content/channel/recommendation/cold-start`，但 spec 中未引用此接口。
...
| 位置 | 路径 |
|------|------|
| 前端 PRD Section 5.1 | `/api/v1/content/channel/search` |
| 前端 design.md | `/api/v1/content/channel/search` |
| 前端 spec (channel-search) | `/api/v1/content/channel/search/query`（已修正） |
| 后端实际端点 | `GET /api/v1/content/channel/search/query` |

spec 已修正为正确路径，但 PRD 和 design.md 仍引用旧路径。前端实现时可能因参考不同文档而产生混淆。

**建议**: 将前端 PRD 和 design.md 中的搜索路径统一修正为 `/api/v1/content/channel/search/query`。
...
| 位置 | 路径 |
|------|------|
| 前端 PRD Section 5.1 | `/api/v1/content/channel/ranking/list`（单一接口，参数 type/dimension） |
...
| 位置 | 新增路径 | 编辑路径 |
|------|---------|---------|
| 前端 PRD Section 5.1 | `/api/v1/content/channel/category/add` | `/api/v1/content/channel/category/edit` |
| 后端实际端点 | `/api/v1/content/channel/category/create` | `/api/v1/content/channel/category/update` |
...
| 编号 | API | 当前状态 | 验证位置 |
|------|-----|---------|---------|
| ~~BI-1~~ | `GET /api/v1/content/channel/discovery/home` | ✅ 已实现 | `ContentChannelDiscoveryController.java:24` |
| ~~BI-2~~ | `POST /api/v1/content/channel/category/enable` | ✅ 已实现 | `ContentChannelCategoryController.java:53` |
| ~~BI-3~~ | `POST /api/v1/content/channel/tag/update` | ✅ 已实现 | `ContentChannelTagController.java:37` |
| ~~BI-4~~ | `POST /api/v1/content/channel/search/feedback` | ✅ 已实现 | `ContentChannelSearchController.java:35` |
| BI-5 | `GET /api/v1/content/channel/editorial-pick/page` | ❌ 仍缺失 | `IContentChannelEditorialPickService` 仅有 `listActivePicks()` |
...
| 位置 | 路径 |
|------|------|
| 前端 PRD Section 5.1 | `/api/v1/content/channel/browse/list` |
| verification-review.md | `/api/v1/content/channel/browse/category` |
| 后端实际端点 | `GET /api/v1/content/channel/browse/category` |

需统一为 `/api/v1/content/channel/browse/category`。
...
|---------------|-------------|---------|
| `GET /api/v1/content/channel/discovery/home` | 缺失 Controller（BI-1） | 不匹配 |
| `GET /api/v1/content/channel/category/tree` | 已存在 | 匹配 |
| `POST /api/v1/content/channel/category/create` | 已存在（PRD 引用 /add） | 路径偏差 |
| `POST /api/v1/content/channel/category/update` | 已存在（PRD 引用 /edit） | 路径偏差 |
| `POST /api/v1/content/channel/category/disable` | 已存在 | 匹配 |
| `POST /api/v1/content/channel/category/enable` | 缺失（BI-2） | 不匹配 |
| `GET /api/v1/content/channel/tag/list` | 已存在 | 匹配 |
| `POST /api/v1/content/channel/tag/add` | 已存在（create） | 匹配 |
| `POST /api/v1/content/channel/tag/edit` | 缺失（BI-3） | 不匹配 |
| `POST /api/v1/content/channel/tag/delete` | 已存在 | 匹配 |
| `GET /api/v1/content/channel/recommendation/list` | 已存在 | 匹配 |
| `GET /api/v1/content/channel/recommendation/cold-start` | 已存在 | 匹配 |
| `POST /api/v1/content/channel/recommendation/not-interested` | 已存在 | 匹配 |
| `GET /api/v1/content/channel/ranking/hot` | 已存在 | 匹配 |
| `GET /api/v1/content/channel/ranking/new` | 已存在 | 匹配 |
| `GET /api/v1/content/channel/ranking/system` | 已存在 | 匹配 |
| `GET /api/v1/content/channel/editorial-pick/list` | 已存在 | 匹配 |
| `GET /api/v1/content/channel/search/query` | 已存在 | 匹配（PRD 引用 /search） |
| `GET /api/v1/content/channel/browse/category` | 已存在（方法体未实现） | 部分匹配 |
