...
- `ContentUserSettingsController` 提供 `/api/v1/content/user/settings/privacy/update`、`/api/v1/content/user/settings/notification`、`/api/v1/content/user/settings/notification/update` 三个接口。
...
**Decision**: 后端只提供 `/api/v1/content/user/settings/security` 返回安全功能列表和各功能的状态（是否已启用），前端据此渲染统一页面。具体安全操作跳转到已有接口。
