...
- **描述**: 审核队列前端封装路径为 `/api/v1/content/channel/review/*`，但后端实际路径为 `/api/v1/content/channel/review/*`；频道治理前端封装路径为 `/channel/governance/*`，后端实际为 `/channel/governance`。路径映射表中的"前端封装路径"列不够精确
...
|---|----------|-------------|-------------|------|
| 1 | 创建频道 | `POST /api/v1/content/channels/create` | `POST /api/v1/channels` | **路径不匹配** |
| 2 | 创建系统频道 | `POST /api/v1/content/admin/channels/create-system` | `POST /api/v1/content/admin/channels/create-system` | OK |
| 3 | 我的频道列表 | `GET /api/v1/content/channels/list` | 后端未定义 | **完全缺失** |
| 4 | 频道详情 | `GET /api/v1/content/channels/{id}` | `GET /api/v1/content/channels/{id}` | OK |
| 5 | 更新频道 | `PUT /api/v1/content/channels/{id}` | `PUT /api/v1/content/channels/{id}` | OK |
| 6 | 删除频道 | `DELETE /api/v1/content/channels/{id}` | `DELETE /api/v1/content/channels/{id}` | OK |
| 7 | 撤销删除 | `POST /api/v1/content/channels/{id}/cancel-delete` | `POST /api/v1/content/channels/{id}/cancel-delete` | OK |
| 8 | 发起转让 | `POST /api/v1/content/channels/{id}/transfer` | `POST /api/v1/content/channels/{id}/transfer` | OK |
| 9 | 确认转让 | `POST /api/v1/content/channels/transfer/{transferId}/confirm` | `POST /api/v1/content/channels/transfer/{transferId}/confirm` | OK |
| 10 | 拒绝转让 | `POST /api/v1/content/channels/transfer/{transferId}/reject` | `POST /api/v1/content/channels/transfer/{transferId}/reject` | OK |
| 11 | 后台频道列表 | `GET /api/v1/content/admin/channels/list` | `GET /api/v1/content/admin/channels/list` | OK |
| 12 | 审核频道 | `POST /api/v1/content/admin/channels/{id}/review` | `POST /api/v1/content/admin/channels/{id}/review` | OK |
| 13 | 审核队列 | `GET /api/v1/content/channel/review/list` | `GET /api/v1/content/channel/review/list` | **路径不匹配** |
| 14 | 名称唯一性校验 | `GET /api/v1/content/channels/check-name` | 后端未定义 | **完全缺失** |
| 15 | 删除前置校验 | `GET /api/v1/content/channels/{id}/delete-check` | 后端未定义 | **完全缺失** |

**额外依赖（前端 design.md 提到但未在 PRD 接口清单中列出）**:

| API | 状态 | 说明 |
|-----|------|------|
| 转让历史查询 `GET /api/v1/content/channels/{id}/transfers` | **完全缺失** | 前端 specs channel-transfer 引用 |
| 待确认转让查询 `GET /api/v1/content/channels/{id}/transfer/pending` | **完全缺失** | 前端 specs channel-transfer 引用 |
| 用户搜索（转让）`GET /api/v1/content/channels/search-user` | **未确认** | 前端 PRD 引用，假设已有 |
...
- **实际情况**: 经核查后端 ChannelController.java 源码，以下 5 个 API **全部已实现**：
  1. `GET /api/v1/content/channels/list` — `ChannelController.listMyChannels()` ✅
  2. `GET /api/v1/content/channels/{id}/delete-check` — `ChannelController.checkDeletePrecondition()` ✅
  3. `GET /api/v1/content/channels/{id}/transfers` — `ChannelController.getTransferHistory()` ✅
  4. `GET /api/v1/content/channels/{id}/transfer/pending` — `ChannelController.getPendingTransfer()` ✅
  5. `GET /api/v1/content/channels/check-name` — `ChannelController.checkNameUnique()` ✅
...
- **实际情况**: 经核查后端 ChannelController.java 源码，`@PostMapping("/create")` 注解确认后端实际路径为 `POST /api/v1/content/channels/create`（含 `/create` 后缀），specs 中的路径是正确的
- **修复**: PRD 中的创建频道路径已从 `POST /api/v1/channels` 更正为 `POST /api/v1/content/channels/create`

#### ~~BLOCK-003: specs 中审核队列 API 路径与后端实际路径不匹配~~ → **已解决**
- **位置**: `specs/review-queue/spec.md`:API 路径注释
- **实际情况**: specs 引用 `GET /api/v1/content/channel/review/list` 与后端 ChannelReviewController 实际路径完全一致。design.md 中的前端封装路径已更正，标注该路径为绝对路径不经过 defHttp 前缀拼接
...
- **实际情况**: PRD 接口清单已与后端实际 Controller 路径对齐，创建频道路径更正为 `POST /api/v1/content/channels/create`，补充了转让历史查询和待确认转让查询两个缺失接口
...
| 后端定义的 API | 前端引用 | 状态 |
|---------------|---------|------|
| `POST /api/v1/channels` | `POST /api/v1/content/channels/create` | **路径不匹配** |
| `GET /api/v1/content/channels/{id}` | `GET /api/v1/content/channels/{id}` | OK |
| `PUT /api/v1/content/channels/{id}` | `PUT /api/v1/content/channels/{id}` | OK |
| `DELETE /api/v1/content/channels/{id}` | `DELETE /api/v1/content/channels/{id}` | OK |
| `POST /api/v1/content/channels/{id}/cancel-delete` | `POST /api/v1/content/channels/{id}/cancel-delete` | OK |
| `POST /api/v1/content/channels/{id}/transfer` | `POST /api/v1/content/channels/{id}/transfer` | OK |
| `POST /api/v1/content/channels/transfer/{id}/confirm` | `POST /api/v1/content/channels/transfer/{transferId}/confirm` | OK |
| `POST /api/v1/content/channels/transfer/{id}/reject` | `POST /api/v1/content/channels/transfer/{transferId}/reject` | OK |
| `POST /api/v1/content/admin/channels/create-system` | `POST /api/v1/content/admin/channels/create-system` | OK |
| `POST /api/v1/content/admin/channels/{id}/review` | `POST /api/v1/content/admin/channels/{id}/review` | OK |
| `GET /api/v1/content/channel/review/list` | `GET /api/v1/content/channel/review/list` | OK |
| `POST /api/v1/content/channel/review/action` | `POST /api/v1/content/admin/channels/{id}/review` | **功能重叠** |
| 后端未定义 | `GET /api/v1/content/channels/list` | **缺失** |
| 后端未定义 | `GET /api/v1/content/channels/{id}/delete-check` | **缺失** |
| 后端未定义 | `GET /api/v1/content/channels/{id}/transfers` | **缺失** |
| 后端未定义 | `GET /api/v1/content/channels/{id}/transfer/pending` | **缺失** |
| 后端未定义 | `GET /api/v1/content/channels/check-name` | **缺失** |
| 后端未定义 | `GET /api/v1/content/channels/search-user` | **未确认** |
...
- **缺失 API**:
  1. `GET /api/v1/content/channels/list` — 我的频道列表
  2. `GET /api/v1/content/channels/{id}/delete-check` — 删除前置校验
  3. `GET /api/v1/content/channels/{id}/transfers` — 转让历史
  4. `GET /api/v1/content/channels/{id}/transfer/pending` — 待确认转让
  5. `GET /api/v1/content/channels/check-name` — 名称唯一性校验
- **影响**: 前端 apply 后这 5 个接口无法调用
- **建议**: 在后端 change 中补充 API 设计和实现

#### BLOCK-C002: 创建频道 API 路径不一致
- **前端 Spec**: `specs/channel-creation/spec.md`
- **引用 API**: `POST /api/v1/content/channels/create`
...
- **引用 API**: `GET /api/v1/content/channel/review/list`
- **后端实际**: `GET /api/v1/content/channel/review/list`
- **影响**: 路径前缀不一致将导致请求失败
...
#### P0 依赖项状态（全部已解决）
- [已实现] `GET /api/v1/content/channels/list` — ChannelController.listMyChannels()
- [已实现] `GET /api/v1/content/channels/{id}/delete-check` — ChannelController.checkDeletePrecondition()
- [已实现] `GET /api/v1/content/channels/{id}/transfers` — ChannelController.getTransferHistory()
- [已实现] `GET /api/v1/content/channels/{id}/transfer/pending` — ChannelController.getPendingTransfer()
- [已实现] `GET /api/v1/content/channels/check-name` — ChannelController.checkNameUnique()

> [已实现] 后台频道列表 `GET /api/v1/content/admin/channels/list` — ChannelAdminController.listAllChannels()，支持 channelType/status/keyword 过滤，分页默认 20 条。
