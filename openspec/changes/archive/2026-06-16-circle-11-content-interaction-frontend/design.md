...
### D6: 拆分为三个独立 Store 分别管理频道治理、审核和圈子状态

实际实现将统一 Store 拆分为三个独立 Store，各司其职：

1. **`src/store/modules/channelGovernance.ts`** — `useChannelGovernanceStore`
   - 管理：内容列（分页/筛选）、置顶/精华切换、删除内容、移出频道、编辑协助、回收站、治理日志
   - 治理操作通过 `executeGovernance` API 统一入口

2. **`src/store/modules/channelReview.ts`** — `useChannelReviewStore`
   - 管理：审核列表（待审区）、批准/拒绝单条及批量操作、审核统计（总数/超时数）

3. **`src/store/modules/circle.ts`** — `useCircleStore`
   - 管理：当前圈子状态、用户角色（isCreator/isModerator/isMember）、权限判断（canMute/canRemove 等）

理由：这些状态跨页面共享（如公告在内容列表页展示，审核统计在管理入口角标展示），提升到全局 Store 避免重复请求。拆分后每个 Store 职责单一，避免单一 Store 膨胀。内容列表的置顶/精华状态通过 API 实时查询，不缓存到 Store。
...
- **[风险] @成员查询无独立接口** → 后端无 `mentionable-members` Controller 端点，前端复用 `GET /api/v1/content/circle/member/list` 获取成员列表。缓解：接口已存在，功能可用，后续可按需添加独立端点。
