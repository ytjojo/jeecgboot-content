...
| 阻塞标记数 | 1 | badge-display spec 中 authorBadges 仍标记 [阻塞]（其余 3 个已移除） |
...
✅ **已验证为误报**：PRD 第 5.1-5.3 节已同步为正确路径（`/api/v1/content/user/growth/...`），与后端实际端点一致。specs 和 tasks 中的 API 路径与 PRD 当前内容一致。
...
| levelChanged 字段 | growth-level: (已移除) | Issue 3: 已解决 | ✅ 已修复 |
| requestId 幂等 | point-system: (已移除) | Issue 1: 已解决 | ✅ 已修复 |
| 用户级衰减状态 | decay-notice: (已移除) | Issue 4: 已解决 | ✅ 已修复 |

3 个已解决的后端 issue 的阻塞标记已移除，仅保留 badge-display 中 authorBadges 的真实阻塞标记。

**ADVISORY-2: PRD 第 5.4 节 API 封装示例代码与实际实现可能不一致**

~~PRD 中的示例代码使用旧路径（如 `getBadgeList`、`getPointBalance`、`getLevelInfo`），而 tasks.md 已更新为新函数名（`getBadgeCatalog`、`getPointLedger`、`getGrowthSummary`）。PRD 示例代码应同步更新。~~ ✅ 已验证：PRD 5.4 节已使用正确函数名，此标记为误报。
...
| 方法 | 路径 | 来源 | 后端实际 | 匹配 |
|------|------|------|---------|------|
| GET | `/api/v1/content/user/growth/badge/catalog` | tasks 1.1 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/badge/detail` | tasks 1.1 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/badge/worn` | tasks 1.1 | ContentUserGrowthController | 匹配 |
| POST | `/api/v1/content/user/growth/badge/wear` | tasks 1.1 | ContentUserGrowthController | 匹配 |
| POST | `/api/v1/content/user/growth/badge/recycle` | tasks 1.1 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/point/ledger` | tasks 1.2 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/point/exchange/goods` | tasks 1.2 | ContentUserGrowthController | 匹配 |
| POST | `/api/v1/content/user/growth/point/exchange` | tasks 1.2 | ContentUserGrowthController | 匹配 |
| POST | `/api/v1/content/user/growth/point/feature/unlock` | tasks 1.2 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/point/feature/unlock` | tasks 1.2 | ContentUserGrowthController | 匹配 |
| POST | `/api/v1/content/user/growth/point/gift/send` | tasks 1.2 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/summary` | tasks 1.3 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/level/config` | tasks 1.3 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/level/benefit` | tasks 1.3 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/decay/rule` | tasks 1.3 | ContentUserGrowthController | 匹配 |
| GET | `/api/v1/content/user/growth/decay/status` | backend-issues Issue 4 | ContentUserGrowthController | 匹配（新增） |
...
✅ **已验证为误报**：PRD 第 5.1-5.3 节已同步为正确路径。

### 后端已有但前端未引用的 API

| 端点 | 功能 | 建议 |
|------|------|------|
| `POST /api/v1/content/user/growth/record` | 记录积分与成长行为 | 内部 API，前端无需封装（合理） |
...
| ID | 问题 | 影响 | 建议 |
|----|------|------|------|
| FLAG-1 | PRD 第 5.1-5.3 节 API 路径与实际后端不一致 | ~~开发参考 PRD 时可能使用错误路径~~ | ✅ 已验证：PRD 已同步为正确路径，此标记为误报 |
| FLAG-2 | 3 个已解决的后端 issue 在 specs 中仍标记 [阻塞] | ~~文档误导~~ | ✅ 已修复：point-system/growth-level/decay-notice 三个 spec 已移除阻塞标记 |
| FLAG-3 | PRD 第 5.4 节 API 封装示例代码使用旧函数名 | ~~开发参考时可能使用错误函数名~~ | ✅ 已验证：PRD 5.4 节已使用正确函数名（getBadgeCatalog/getPointLedger/getGrowthSummary），此标记为误报 |
...
1. ~~**同步更新 PRD 第 5 节 API 路径**（FLAG-1/FLAG-3）~~：✅ 已验证 PRD 已同步为正确路径
2. ~~**更新 spec 阻塞标记**（FLAG-2）~~：✅ 已修复，3 个阻塞标记已移除
