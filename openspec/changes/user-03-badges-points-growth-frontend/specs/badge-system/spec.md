...
- **WHEN** 用户点击"保存"按钮
- **THEN** 调用 POST `/content/user/growth/badge/wear` API（实际端点），成功后提示"佩戴设置已更新"并退出编辑模式
...
- **THEN** 二次确认"确认回收该勋章？此操作不可撤销"，确认后调用 POST `/content/user/growth/badge/recycle` API（实际端点）
