...
#### Scenario: 加载当前公告
- **WHEN** 管理员进入公告管理页
- **THEN** 调用 `/api/channel/announcement/{channelId}` 获取当前公告，展示公告状态（已发布/未发布）和公告内容
...
#### Scenario: 预览公告
- **WHEN** 管理员点击"预览"
- **THEN** 在下方预览区展示渲染效果，通过 `/api/channel/announcement/preview` 接口过滤不安全内容
...
- **THEN** 调用 `/api/channel/announcement/{channelId}/history` 展示最近 3 个历史版本（版本号、修改人、修改时间），支持"恢复此版本"操作

#### Scenario: 恢复历史版本
- **WHEN** 管理员点击某历史版本的"恢复此版本"
- **THEN** 调用 `/api/channel/announcement/restore/{versionId}` 恢复为当前公告
