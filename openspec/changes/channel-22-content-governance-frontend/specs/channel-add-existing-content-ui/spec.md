...
- **THEN** 展示内容搜索框，调用 `GET /api/v1/content/channel/publish/add-existing/search` 搜索可添加的已发布内容（**后端待实现**），展示搜索结果列表
...
#### Scenario: 提交后展示结果
- **WHEN** 用户点击"添加到频道"
- **THEN** 调用 `POST /api/v1/content/channel/publish/add-existing` API，提交后逐频道展示结果反馈
