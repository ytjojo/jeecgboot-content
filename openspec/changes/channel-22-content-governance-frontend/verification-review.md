...
| 控制器 | 路径前缀 | 文件 |
|--------|----------|------|
| ChannelPublishController | `/content/channel/publish` | `controller/ChannelPublishController.java` |
| ChannelContentReviewController | `/content/channel/review` | `controller/ChannelContentReviewController.java` |
| ChannelReviewController | `/jeecg-boot/api/v1/content/channel/review` | `controller/ChannelReviewController.java` |
| ChannelContentGovernanceController | `/content/channel/governance` | `controller/ChannelContentGovernanceController.java` |
| ChannelGovernanceController | `/channel/governance` | `controller/ChannelGovernanceController.java` |
| ChannelAnnouncementController | `/content/channel/announcement` | `controller/ChannelAnnouncementController.java` |
...
| `/api/channel/publish/submit` | `POST /content/channel/publish` | 路径偏差 |
...
| spec 引用路径 | 实际后端路径 | 状态 |
|---------------|-------------|------|
| `/api/channel/review/list` | `GET /jeecg-boot/api/v1/content/channel/review/list` | 路径偏差（两处） |
| `/api/channel/review/approve` | `POST /content/channel/review` (action=APPROVE) | 架构差异 |
| `/api/channel/review/reject` | `POST /content/channel/review` (action=REJECT) | 架构差异 |
| `/api/channel/review/stats` | **不存在** | 缺失 - 无审核统计端点 |

**架构差异说明**: spec 设计为独立的 approve/reject 端点，实际后端使用统一的 `POST /content/channel/review` 端点 + `action` 字段区分操作。另外存在两个审核控制器：
...
| `/api/channel/governance/pin` | `POST /content/channel/governance` (action=PIN) | 架构差异 |
| `/api/channel/governance/feature` | `POST /content/channel/governance` (action=FEATURE) | 架构差异 |
| `/api/channel/governance/delete` | `POST /content/channel/governance` (action=DELETE) | 架构差异 |
| `/api/channel/governance/move` | `POST /content/channel/governance` (action=MOVE) | 架构差异 |
| `/api/channel/governance/edit-assist` | `POST /content/channel/governance` (action=EDIT_ASSIST) | 架构差异 |
| `/api/channel/governance/recycle-bin/list` | **不存在** | 缺失 - 回收站列表查询无端点 |
| `/api/channel/governance/recycle-bin/restore` | `POST /content/channel/governance` (action=RESTORE) | 架构差异 |
| `/api/channel/governance/log/list` | **不存在** | 缺失 - 治理日志查询无端点 |
| `/api/channel/governance/edit-assist/history/{contentId}` | **不存在** | 缺失 - 编辑协助历史查询无端点 |

**架构差异说明**: 实际后端将所有治理操作（PIN/UNPIN/FEATURE/UNFEATURE/DELETE/RESTORE/MOVE/EDIT_ASSIST）统一到 `POST /content/channel/governance` 一个端点，通过 `ChannelGovernanceReq.action` 字段区分。spec 设计为多个独立端点。

#### 公告模块

| spec 引用路径 | 实际后端路径 | 状态 |
|---------------|-------------|------|
| `GET /api/channel/announcement/{channelId}` | `GET /content/channel/announcement/channel/{channelId}` | 路径偏差 |
| `POST /api/channel/announcement` | `POST /content/channel/announcement` | 路径偏差 |
| `PUT /api/channel/announcement/{id}` | `PUT /content/channel/announcement/{id}` | 路径偏差 |
| `DELETE /api/channel/announcement/{id}` | `DELETE /content/channel/announcement/{id}` | 路径偏差 |
...
| `/api/channel/content/add` | `POST /content/channel/publish/add-existing` | 路径偏差 |
...
所有 spec 文件中的 API 路径使用 `/api/channel/...` 格式，但实际后端路径为 `/content/channel/...`。这是系统性偏差，需要统一修正。
...
**治理操作**: spec 设计了 `/pin`、`/feature`、`/delete`、`/move`、`/edit-assist` 等独立端点，实际统一为 `POST /content/channel/governance` + `action` 字段。

**审核操作**: spec 设计了 `/approve`、`/reject` 独立端点，实际统一为 `POST /content/channel/review` + `action` 字段。
...
存在两个审核控制器：
- `ChannelContentReviewController` (`/content/channel/review`) - 处理内容发布审核（APPROVE/REJECT）
- `ChannelReviewController` (`/jeecg-boot/api/v1/content/channel/review`) - 处理频道创建审核（approved/rejected/returned）
...
| `/api/channel/review/list` | `/jeecg-boot/api/v1/content/channel/review/list` |
| `/api/channel/review/approve` | `/content/channel/review` (body: `{reviewId, action: "APPROVE"}`) |
| `/api/channel/review/reject` | `/content/channel/review` (body: `{reviewId, action: "REJECT", rejectReason}`) |
| `/api/channel/governance/pin` | `/content/channel/governance` (body: `{channelId, contentId, action: "PIN"}`) |
