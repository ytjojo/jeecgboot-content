...
| 一致性 (Consistency) | 5/10 | 3 | 2 | 1 |
| 可实现性 (Feasibility) | 8/10 | 0 | 1 | 1 |
| 可测试性 (Testability) | 7/10 | 0 | 2 | 1 |
| 接口契约 (API Contract) | 4/10 | 3 | 2 | 0 |
| 边界覆盖 (Boundary) | 8/10 | 0 | 1 | 2 |
| **综合** | **6.5/10** | **7** | **10** | **6** |
...
| API 契约完整率 | 43% | 3/7 个 API 端点的路径/类型在 plan.md 中与后端完全一致 |
...
| POST /api/circle/recommend/exposure | ❌ 后端待开发 | 中（可 mock 开发） |
...
测试中 `flushWithBeacon` 发送到 `/content/circle/recommend/exposure`，但实际后端路径应为 `/api/circle/recommend/exposure`（且该接口尚不存在）。
...
**BLOCK-5: PRD API 路径全部使用 `/content/circle/` 前缀，与后端不一致**

| 功能 | PRD 路径 | 后端实际路径 | plan.md 路径 |
|------|----------|-------------|-------------|
| 统计数据查询 | `/content/circle/{id}/analytics` | `/api/circle/{id}/data/statistics` | `/api/circle/{id}/data/statistics` ✅ |
| 统计数据导出 | `/content/circle/{id}/analytics/export` | `/api/circle/{id}/data/export` | `/api/circle/{id}/data/export` ✅ |
| 推荐列表 | `/content/circle/recommend` | `/api/circle/recommend` | `/api/circle/recommend` ✅ |
| 推荐曝光 | `/content/circle/recommend/exposure` | ❌ 不存在 | `/api/circle/recommend/exposure` |
| 推荐点击 | `/content/circle/recommend/click` | `/api/circle/recommend/click` | `/api/circle/recommend/click` ✅ |
| 热门榜 | `/content/circle/ranking/hot` | `/api/circle/ranking/hot` | `/api/circle/ranking/hot` ✅ |
| 新锐榜 | `/content/circle/ranking/new` | `/api/circle/ranking/new` | `/api/circle/ranking/new` ✅ |

plan.md 已修正为后端实际路径（6/7 正确），但 PRD 仍使用错误路径，需同步修正。
...
| 推荐曝光 | plan.md: `POST /api/circle/recommend/exposure` | ❌ 不存在 | ❌ |
...
### BLOCK 问题（7 个，必须在 apply 前修复）

| ID | 问题 | 影响 |
|----|------|------|
| BLOCK-1 | PRD "上期计算规则"和"基数为零边界处理"未在 plan.md 中实现 | 核心指标卡片功能缺失 |
| BLOCK-2 | DataAnalyticsPanel.vue 使用 PRD 字段名（memberTotal 等），与 types.ts 不一致 | 编译报错 |
| BLOCK-3 | TrendChart 数据源类型不匹配（TrendDataPoint vs DailyTrend） | 编译报错 |
| BLOCK-4 | RankCircleCard/RankList 引用不存在的字段（id, iconUrl, name） | 编译报错 |
| BLOCK-5 | PRD API 路径全部使用 `/content/circle/` 前缀 | PRD 与实现不一致 |
| BLOCK-6 | 前端类型定义与后端响应结构多处不匹配 | 运行时数据解析失败 |
| BLOCK-7 | sendBeacon URL 使用错误路径 `/content/circle/` | 曝光上报失败 |
...
| FLAG-2 | 数据统计页权限校验仅有 TODO 占位 |
| FLAG-3 | 响应式布局无 spec 覆盖 |
| FLAG-4 | 测试框架使用 Jest，项目已迁移 Vitest |
| FLAG-5 | 组件测试缺失（StatCard、TrendChart、RecommendationPanel 等） |
| FLAG-6 | 测试 mock 路径使用 Jest 语法 |
| FLAG-7 | 未覆盖快速 Tab 切换导致的请求竞态 |
...
|----|------|
| ADVISORY-1 | 429 和 404 错误码处理未在 plan.md 中体现 |
| ADVISORY-2 | 缓存策略（5 分钟过期）未在组件中调用 |
| ADVISORY-3 | useChart 使用全量引入，应按需引入 |
| ADVISORY-4 | sendBeacon URL 路径不一致 |
| ADVISORY-5 | 未覆盖"上期值为 0 且本期值大于 0"的边界 |
| ADVISORY-6 | 未覆盖 ECharts 容器尺寸为 0 的边界 |
