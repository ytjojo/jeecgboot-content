...
5. **曝光上报接口**: 后端 CircleRecommendController 缺失 `POST /api/circle/recommend/exposure` 接口，需后端补充
6. **iconUrl 字段**: 后端 CircleRecommendItem 和 CircleRankingItem 均缺少 iconUrl 字段，前端需确认是否使用默认图标或等待后端补充
...
**已确认存在的后端接口**:
- `GET /api/circle/{circleId}/data/statistics` — CircleDataController（统计数据查询）
- `GET /api/circle/{circleId}/data/export` — CircleDataController（CSV 导出）
- `GET /api/circle/recommend` — CircleRecommendController（推荐列表）
- `POST /api/circle/recommend/click` — CircleRecommendController（点击上报）
- `GET /api/circle/ranking/hot` — CircleRankingController（热门榜）
- `GET /api/circle/ranking/new` — CircleRankingController（新锐榜）

**缺失的后端接口**:
- `POST /api/circle/recommend/exposure` — 曝光批量上报（需后端开发）

**前端需适配的差异**:
- 统计数据响应使用 `CircleDataStatisticsVO`（无变化百分比字段，前端需自行计算）
- 推荐列表响应包装在 `CircleRecommendVO.items` 中（非直接数组）
- 推荐 click 接口参数为 `sourceId`（非 `circleId`）
