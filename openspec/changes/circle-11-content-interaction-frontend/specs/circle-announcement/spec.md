...
- **THEN** 调用 `POST /circle-announcement/` 接口（body: `{ circleId, content, expireAt }`），成功后关闭弹窗，圈子顶部展示新公告，Toast 提示"公告已发布"
...
#### Scenario: 有生效公告时展示
- **WHEN** 圈子有生效公告
- **THEN** 调用 `GET /circle-announcement/active/{circleId}` 接口获取公告，内容列表页顶部展示公告栏，显示公告摘要（最多 2 行，超出省略）和有效期
...
#### Scenario: 管理员删除公告
- **WHEN** 管理员点击"删除"
- **THEN** 确认框"确认删除该公告？"，确认后调用 `DELETE /circle-announcement/{id}` 接口，成功后公告栏消失，Toast 提示"公告已删除"
