...
选择为每个内容提供独立的 PUT 切换接口（`/circle-content/{id}/pin`、`/circle-content/{id}/featured`），而非批量更新接口。两个接口均需要 `circleId` 查询参数（`@RequestParam circleId`）。
...
- **[权衡] 禁言时长前端固定 vs 后端配置** → 当前假设前端固定选项（1小时/1天/7天/30天/永久），灵活性受限但实现简单。如需动态配置，后续迭代调整。注：后端当前不接受禁言时长参数。
