...
- **[风险] 后端 API 部分缺失** → @成员查询、公告删除、申请审核计数等接口后端尚未实现，详见 `backend-issues.md`。缓解：前端先对接已实现接口，缺失接口待后端补充后联调。
- **[权衡] 禁言时长前端固定 vs 后端配置** → 当前假设前端固定选项（1小时/1天/7天/30天/永久），灵活性受限但实现简单。如需动态配置，后续迭代调整。注：后端当前不接受禁言时长参数，`POST /circle-report/{reportId}/mute?circleId=xxx` 接口无 `duration` body 参数。降级方案：前端禁言弹窗仍展示时长选择 UI，但提交时不传 duration 参数（后端忽略），待后端实现后再对接。
