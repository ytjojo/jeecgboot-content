...
本次变更需要在现有前端架构下新增完整的圈子模块，包含 6 个页面、8 个业务组件、1 个 Pinia Store、14 个 API 接口对接。后端接口已由 PRD 定义（`/content/circle/*`），前端需按接口规范实现全部交互。
...
**[R1] 后端接口未就绪** → 前端先基于接口文档定义 TypeScript 类型和 Mock 数据开发，接口就绪后切换真实调用。API 层独立封装，切换成本低。
