...
本次变更需要在现有前端架构下新增完整的圈子模块，包含 6 个页面、8 个业务组件、1 个 Pinia Store、15 个 API 接口定义（全部已实现）。
...
**后端实际路径**（2026-06-09 最终版，参数风格全部统一）:

| 功能 | 后端实际路径 | 方法 | 前端封装名称 | 请求参数 |
|------|------------|------|------------|---------|
| 创建圈子 | `/api/v1/content/circle/create` | POST | `createCircle` | Body: `CircleCreateReq` |
| 更新圈子 | `/api/v1/content/circle/update` | PUT | `updateCircle` | Body: `CircleUpdateReq` |
| 圈子详情 | `/api/v1/content/circle/{id}` | GET | `getCircleDetail` | Path: `id` |
| 加入圈子 | `/api/v1/content/circle/join` | POST | `joinCircle` | Body: `CircleJoinReq` |
| 退出圈子 | `/api/v1/content/circle/leave` | POST | `leaveCircle` | Body: `CircleLeaveReq` |
| 名称校验 | `/api/v1/content/circle/check-name` | GET | `checkCircleName` | Query: `name` |
| 我的列表 | `/api/v1/content/circle/my-list` | GET | `getMyCircleList` | Query: `pageNum`, `pageSize` |
| 公开列表 | `/api/v1/content/circle/public-list` | GET | `getPublicCircleList` | Query: `pageNum`, `pageSize` |
| 成员列表 | `/api/v1/content/circle/member/list` | GET | `getMemberList` | Query: `circleId`, `role?`, `status?`, `pageNum`, `pageSize` |
| 变更角色 | `/api/v1/content/circle/member/change-role` | POST | `changeRole` | Body: `CircleMemberUpdateReq` |
| 禁言 | `/api/v1/content/circle/member/mute` | POST | `muteMember` | Body: `CircleMemberUpdateReq` |
| 解除禁言 | `/api/v1/content/circle/member/unmute` | POST | `unmuteMember` | Body: `CircleMemberUpdateReq` |
| 移除成员 | `/api/v1/content/circle/member/remove` | POST | `removeMember` | Body: `CircleMemberUpdateReq` |
| 搜索 | `/api/v1/content/circle/search` | GET | `searchCircle` | Query: `keyword`, `pageNum`, `pageSize`；返回 `Page<CircleSearchResultVO>` |
| 治理日志 | `/api/v1/content/circle/governance-log/list` | GET | `getGovernanceLogList` | Query: `circleId`, `pageNum`, `pageSize` |

**当前状态**: ✅ 15 个接口全部已实现，参数风格统一（所有写操作 Body、所有读操作 Query/Path、所有列表返回 Page），无需 Mock 开发。

**理由**: 直接对齐后端路径避免中间转换层的维护成本和 404 风险。join 和 leave 在 CircleController 中（`/api/v1/content/circle/` 前缀），不在 CircleMemberController 中（`/api/v1/content/circle/member/` 前缀），因为加入/退出是圈子级操作而非纯成员管理操作。

**前端实现要点**:
- 详情接口使用 Path 参数 `/{id}`
- 所有写操作统一 `@RequestBody`（create/update/join/leave/change-role/mute/unmute/remove）
- 成员列表支持 `role`/`status` 可选筛选，默认 `pageSize=50`
- 搜索和所有列表接口返回 `Page<T>` 分页对象

### D12: 分页参数与响应格式

**选择**: 所有接口统一使用 `pageNum`/`pageSize` 作为分页请求参数。

**分页参数对照**:

| API | 分页参数 | 默认值 | 说明 |
|-----|---------|--------|------|
| 搜索 (`/search`) | `pageNum`, `pageSize` | 1, 20 | 通过 `CircleSearchReq` 对象，返回 `Page<CircleSearchResultVO>` |
| 我的列表 (`/my-list`) | `pageNum`, `pageSize` | 1, 20 | 直接 Query 参数 |
| 公开列表 (`/public-list`) | `pageNum`, `pageSize` | 1, 20 | 直接 Query 参数 |
| 成员列表 (`/member/list`) | `pageNum`, `pageSize` | 1, 50 | 直接 Query 参数 |
| 治理日志 (`/governance-log/list`) | `pageNum`, `pageSize` | 1, 20 | 直接 Query 参数 |

**分页响应格式**（Page 对象，适用于所有列表接口 my-list/public-list/member-list/search/governance-log）:
...
**理由**: 后端所有 API 已统一使用 `pageNum`/`pageSize`，前端可统一封装分页参数。


## Risks / Trade-offs

**[R1] 后端接口未就绪** → ✅ 已解决。15 个接口全部已实现，无需 Mock 开发。

**[R8] 后端 CircleVO 缺少前端依赖字段** → ✅ 已解决。`applyStatus`（PENDING/APPROVED/REJECTED/null）和 `isInvited`（Boolean）已补充到 CircleVO。前端可直接使用详情接口返回值驱动加入按钮状态。
...
2. **成员上限配置**: PRD 提到默认 500 人可由后端配置覆盖。`maxMemberCount` 已确认等同于 `memberLimit`。
3. **搜索接口返回格式**: 当前返回 `List<CircleSearchResultVO>`（全量列表），是否后续改为分页返回？
