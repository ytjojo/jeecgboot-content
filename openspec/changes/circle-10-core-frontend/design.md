...
本次变更需要在现有前端架构下新增完整的圈子模块，包含 6 个页面、8 个业务组件、1 个 Pinia Store、14 个 API 接口对接。后端已实现 8 个接口（create/update/join/leave/change-role/mute/unmute/remove/search），6 个接口尚未实现（detail/my-list/public-list/check-name/member-list/governance-log），前端需对缺失接口采用 Mock 开发策略。
...
**[R1] 后端接口未就绪** → 前端先基于接口文档定义 TypeScript 类型和 Mock 数据开发，接口就绪后切换真实调用。API 层独立封装，切换成本低。当前后端已实现 8/14 个接口，缺失的 6 个接口（detail/my-list/public-list/check-name/member-list/governance-log）需后端优先补充 P0 级接口（detail/my-list/public-list/member-list）。
