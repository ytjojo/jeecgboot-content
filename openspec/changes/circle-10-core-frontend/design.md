...
| 功能 | 后端实际路径 | 前端封装名称 |
|------|------------|------------|
| 创建圈子 | POST `/api/v1/content/circle/create` | `createCircle` |
| 更新圈子 | PUT `/api/v1/content/circle/update` | `updateCircle` |
| 加入圈子 | POST `/api/v1/content/circle/join` | `joinCircle` |
| 退出圈子 | POST `/api/v1/content/circle/leave` | `leaveCircle` |
| 变更角色 | POST `/api/v1/content/circle/member/change-role` | `changeRole` |
| 禁言 | POST `/api/v1/content/circle/member/mute` | `muteMember` |
| 解除禁言 | POST `/api/v1/content/circle/member/unmute` | `unmuteMember` |
| 移除成员 | POST `/api/v1/content/circle/member/remove` | `removeMember` |
| 搜索 | GET `/api/v1/content/circle/search` | `searchCircle` |
...
**注意点**:
- 加入接口路径：`/api/v1/content/circle/join`（非 `/api/v1/content/circle/member/join`）
- 退出接口：`/api/v1/content/circle/leave`（非 `/api/v1/content/circle/member/quit`），封装名称用 `leaveCircle` 而非 `quitCircle`
- 角色变更：`/api/v1/content/circle/member/change-role`（非 `/api/v1/content/circle/member/set-moderator`）
