...
系统 SHALL 支持展示后端已定义的徽章种类。PRD 定义 6 种徽章（持续创作者、优质贡献者、活跃参与者、圈内新星、内容里程碑、社交达人），实际展示种类由后端 `GET /api/v1/content/user/growth/achievement/list` 接口返回的数据决定。注意：数据库当前仅初始化 4 种徽章（ach_001-004），「内容里程碑」和「社交达人」待后端补充。
