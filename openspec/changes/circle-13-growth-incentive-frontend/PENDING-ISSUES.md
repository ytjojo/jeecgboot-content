...
| 编号 | 问题 | 详情 | 影响 |
|------|------|------|------|
| VO-1 | MemberGrowthVO 缺少 `totalBadges` | PRD 个人成长页需展示"已获得徽章总数" | 前端无法展示徽章总数（可通过 achievement/list 接口变通） |
...
| 编号 | 问题 | 当前默认假设 | 确认方 |
|------|------|------------|--------|
| Q-1 | WebSocket 通知消息体格式 | 假设包含 circleId 和通知类型字段 | 后端 |
| Q-2 | 连续参与 7 天窗口是自然周还是滚动 7 天 | 滚动 7 天（PRD 描述"近 7 天"） | 产品 |
