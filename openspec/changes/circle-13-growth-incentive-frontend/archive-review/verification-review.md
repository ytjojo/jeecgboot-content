...
|------|---------------|---------------|
| 后端 API 路径 | 严重不匹配（4个RESTful路径错误） | ✅ **已确定** — CircleLevelController 使用 `/circle/growth/` 前缀（数据主体是圈子），其余 3 个 Controller 使用 `/user/growth/` 前缀（数据主体是用户）。双前缀是有意设计，不会统一迁移。见 `docs/agent-context/circle-growth-api-conventions.md` |
...
**关键变更**:
1. ✅ CircleLevelVO 已补充 `benefits(List<CircleBenefitVO>)`, `memberScore`, `contentScore`, `activityScore`, `nextLevelConditions(List<LevelConditionVO>)`
...
- CircleLevelVO.benefits: `List<CircleBenefitVO>`（含 `{name, unlocked}`，可区分已解锁/未解锁）
...
**已确认**: CircleLevelController 使用 `/circle/growth/` 前缀是因为数据主体是圈子（只需 circleId），其余 3 个 Controller 保持 `/user/growth/` 前缀是因为数据主体是用户（需要 userId）。双前缀是有意设计，不会统一迁移。详见 `docs/agent-context/circle-growth-api-conventions.md`。
...
private List<CircleBenefitVO> benefits;           // ✅ 新增（{name, unlocked}）
...
**注意**: `benefits` 为 `List<CircleBenefitVO>`（含 `name` + `unlocked`），可直接区分已解锁/未解锁权益。
...
- `benefits` → 后端已提供 `List<CircleBenefitVO> benefits`（`{name, unlocked}`），可直接展示已解锁/未解锁权益列表
...
1. ~~CircleLevelController 迁移至 `/circle/growth/` 后，其余 3 个 Controller 是否会统一迁移？~~ **已确认：不迁移。双前缀是有意设计。**
2. `benefits` 为 `List<CircleBenefitVO>`（`{name, unlocked}`），可直接展示已解锁/未解锁权益。✅ 已确认无需改动。
