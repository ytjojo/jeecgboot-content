...
- 圈子等级: `GET /content/user/growth/level/info?circleId=` (CircleLevelController)
- 成员成长: `GET /content/user/growth/info?circleId=&userId=` (MemberGrowthController)
- 成就徽章: `GET /content/user/growth/achievement/list?circleId=&userId=` (AchievementController)
- 排行榜: `GET /content/user/growth/leaderboard?circleId=&dimension=&period=&currentUserId=` (LeaderboardController)
- 连续参与: `GET /content/user/growth/participation?circleId=&userId=` (MemberGrowthController)
...
- **[后端接口未就绪]** → 前端先用 Mock 数据开发，接口联调时替换。API 封装层做好类型定义，降低联调风险。
- **[WebSocket 通知消息体格式不确定]** → 前端先按假设实现通知监听，预留消息体解析的容错逻辑（字段缺失时降级处理）。
- **[徽章图标资源来源不确定]** → 先按后端返回 URL 实现，前端预留本地兜底图标。
...
| 编号 | 问题 | 影响 | 当前假设 |
|------|------|------|----------|
| Q1 | 圈子等级信息是新增接口还是扩展已有圈子详情接口 | API 对接 | **已确认**: 后端已实现独立接口 `GET /content/user/growth/level/info` |
| Q2 | WebSocket 通知消息体是否包含圈子 ID 和通知类型 | 通知解析 | 包含 |
| Q3 | 排行榜 currentUser 由后端返回还是前端匹配 | 高亮逻辑 | 后端返回 |
| Q4 | 徽章图标由后端返回 URL 还是前端本地维护 | 徽章展示 | 后端返回 URL |
